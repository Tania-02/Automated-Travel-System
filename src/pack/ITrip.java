...
public class ITrip
{
	...
	public static void insert()
	{
		...
		int i=s.executeUpdate("INSERT INTO IFlights(DATE,ORIGIN,DESTINATION,FLIGHT,PRICE,Seats_Availabilty) VALUES('"+date+"','"+origin+"','"+destination+"','"+air+"',"+price+","+seats+")");
		if(i!=0)
...
public static void updatePrice()
	{
		...
		try
		{
			...
			int i=s.executeUpdate("UPDATE IFlights SET PRICE="+p+" where ORIGIN ='"+origin+"' and DESTINATION='"+destination+"'");
			if(i>0)
...
public static void updateSeats()
{
	...
	try
	{
		s=con.createStatement();
		rs=s.executeQuery("select * from IFlights");
		int i=s.executeUpdate("UPDATE IFlights SET SEATS_Availability="+p+" where ORIGIN ='"+origin+"' and DESTINATION='"+destination+"'");
		if(i>0)
			System.out.println("Updated!!");
...
public static void updateDate()
{
	...
	try
	{
		s=con.createStatement();
		rs=s.executeQuery("select * from IFlights");
		int i=s.executeUpdate("UPDATE IFlights SET DATE='"+d+"' where ORIGIN ='"+origin+"' and DESTINATION='"+destination+"'");
		if(i>0)
			System.out.println("Updated!!");
...
public static void updateAirlines()
{
	...
	try
	{
		s=con.createStatement();
		rs=s.executeQuery("select * from IFlights");
		int i=s.executeUpdate("UPDATE IFlights SET FLIGHT='"+air+"' where ORIGIN ='"+origin+"' and DESTINATION='"+destination+"'");
		if(i>0)
			System.out.println("Updated!!");
...
public static void delete()
	{
		...
		try
		{
			s=con.createStatement();
			rs=s.executeQuery("select * from IFlights");
			int i=s.executeUpdate("delete from IFlights where ORIGIN='"+origin+"' and DESTINATION='"+destination+"'");
			if(i>0)
	...
	public static void main(String args[])
	{
		...
		try
		{
			...
			while(true)
			{
				...
				System.out.println("5.TO UPDATE AIRLINES");
