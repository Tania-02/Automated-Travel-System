...
<% id=Integer.parseInt(request.getParameter("id"));
	seat=(int)session.getAttribute("seattour2");
	session.setAttribute("tour2id", id);%>
<%!List<Tour2> list; %>
<%
	list=Tour2DAO.getRecords();
	 int flag1=0;
 ...
 for(Tour2 s:list)
 {
	   ...
	   {	
 			...
 			<table border='1' cellpadding ='4' width='60%'>
   			<tr><th>ID</th><th>DESTINATION</th><th>ORIGIN</th><th>MEALS</th><th>PRICE</th><th>SEATS_Booked</th>
   			<tr><td><%=s.getID()%></td><td><%=s.getDestination()%></td><td><%=s.getOrigin()%></td><td><%=s.getMeal()%></td><td><%=s.getPrice()%></td>
	  		<td><%=seat %><td><a href='Payment.jsp?book=<%= "tour2" %>'><h3>Confirm</h3></a></td></tr>
			<%session.setAttribute("tour2seat",s.getSeat()-seat); 
			/*seat=s.getSeat()-seat;
