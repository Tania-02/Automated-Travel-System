...
<header>
	...
	<%	   String origin=request.getParameter("origin");
		  ...
		  <%!List<Tour1> list; %>
		  <%
		  	list=Tour1DAO.getRecords();
		  	 int flag1=0;

		  	 
		  	 
		  	 //session.getAttributes(id)
		   for(Tour1 s:list)
		   {
			   if(month.equalsIgnoreCase(s.getMonth())&&theme.equalsIgnoreCase(s.getTheme())&&s.getSeat()>0 && origin.equalsIgnoreCase(s.getOrigin()) && dest.equalsIgnoreCase(s.getDestination()))
			   {	
				   if(flag1==0)
					   {%>
					   <table border='1' cellpadding ='4' width='60%'>
					   <tr><th>ID</th><th>DESTINATION</th><th>ORIGIN</th><th>MEALS</th><th>PRICE</th>
					  <% flag1=1;
					  
					   }%>
				  				   
				  <tr><td><%=s.getID()%></td><td><%=s.getDestination()%></td><td><%=s.getOrigin()%></td><td><%=s.getMeal()%></td><td><%=s.getPrice()%></td>
				  <td><a href='booktour1.jsp?id=<%=s.getID()%>'<%session.setAttribute("seattour1",seat);%>>BOOK</a></td></tr>
			<%   }
