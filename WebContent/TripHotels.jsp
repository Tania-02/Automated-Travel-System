...
<header>
		...
		<nav>
			...
			<ul>
				...
				<li><a href="Hotels.html"  class="current">HOTELS</a></li>
...
int r=Integer.parseInt(rooms);
	flag=0;
 list=HotelsDAO.getRecords(star);
   %>
	<table border="1" cellspacing="10" cellpadding="4">
	<tr><th>LOCATION</th><th>NAME</th><th>STAR</th><th>ROOMS</th><th>ADDRESS</th><th>PHONE</th><th>PRICE</th>
	<%
	if(flag==0)
	for(TripH s:list)
	{ 
	 	
 		if( (loc.equalsIgnoreCase(s.getLocation())) && (r<=s.getRooms()) )
 		{ 		 
 			flag=1;%>
 			<tr><td><%=s.getLocation()%></td><td><%= s.getName() %></td><td><%= s.getStar()%></td><td><%= r%></td><td><%= s.getAddress()%></td><td><%=s.getPhone()%></td><td><%= s.getPrice()%></td>
		<td><a href='bookh.jsp?add1=<%=s.getAddress()%>'<%session.setAttribute("star1",star);session.setAttribute("name1",s.getName());session.setAttribute("rooms1",rooms);session.setAttribute("loc1",s.getLocation());session.setAttribute("phone1",s.getPhone());session.setAttribute("price1",s.getPrice());%>>BOOK</a>
    		   </td></tr> 
		  <% 
		}
	}%>
	...
	<br><br><br><br><br><br>
		...
		<%out.print("SORRY!!!NO ROOMS AVAILABLE"); 
     }  %>
   </table>
<footer>
		<section>
