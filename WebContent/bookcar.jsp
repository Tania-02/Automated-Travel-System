...
String p=(String)session.getAttribute("ratecar");
 ...
 for(Cars s:list)
 {
	   ...
	   {	
 			%><table border='1' cellpadding ='4' width='60%'>
  			 <tr><th>DATE</th><th>ORIGIN</th><th>PRICE</th><th>TYPE</th><th>SEATS</th><th>ID</th>
  			 <tr><td><%=s.getDate()%></td><td><%=origin%></td>
   			<%if(p.equalsIgnoreCase("p1"))
   				...
   				<%} %>
   				<td><%=s.getType()%></td>
	 			 <td><%=s.getCars() %><td><%=s.getID()%><td><a href='distance.jsp?id=<%=s.getID()%>'><h3>Confirm</h3></a></td></tr>
				<%	
				 	//session.setAttribute("addcar", date) ;
				   //session.setAttribute("addcar", origin) ;
				   //session.setAttribute("addcar", date) ;
				   //session.setAttribute("origincar", origin) ;//CarsDAO.add(id,s.getPrice2(),s.getPrice(),s.getType(),s.getCars(),s.getDate());
				break;
	   }
 }
	%>
